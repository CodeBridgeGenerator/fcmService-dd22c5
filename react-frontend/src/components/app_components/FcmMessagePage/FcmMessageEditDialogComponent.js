...
from: _entity?.from?._id,
        ...
        await client.service("fcmMessage").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info fcmMessage updated successfully" });
...
const fromOptions = from.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Fcm Message" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            style={{ maxWidth: "55vw" }} role="fcmMessage-edit-dialog-component">
