...
const FcmMessagePage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Fcm Message", type: "error", message: error.message || "Failed get Fcm Message" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("fcmMessage").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Fcm Message", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("fcmMessage").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="fcmMessage-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="fcmMessage-datatable">
                    <FcmMessageDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
