...
const [from, setFrom] = useState([]);
    ...
    useEffect(() => {
        ...
        client
            .service("fcmMessage")
            .get(urlParams.singleFcmMessageId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
...
const from = Array.isArray(res.from)
        ...
        setFrom(from);
            ...
            .catch((error) => {
                console.log({ error });
                props.alert({ title: "FcmMessage", type: "error", message: error.message || "Failed get fcmMessage" });
            });
    }, [props,urlParams.singleFcmMessageId]);
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">From</label>
      ...
      <CommentsSection
        recordId={urlParams.singleFcmMessageId}
        user={props.user}
        alert={props.alert}
        serviceName="fcmMessage"
      />
      <div
        id="rightsidebar"
