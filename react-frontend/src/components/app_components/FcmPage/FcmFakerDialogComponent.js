...
import fcmFakerFactory from "./fcmFakerFactory";
...
const FcmFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [receipientIds, setreceipientIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = fcmFakerFactory(props.user,fakerCount,receipientIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fcm").create(elem));
...
export default connect(mapState, mapDispatch)(FcmFakerDialogComponent);
